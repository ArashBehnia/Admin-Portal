...
function ReasonBadge({ reason }: { reason: string }) {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 rounded text-[11px] font-medium border ${colorClass}`}
    >
      {label}
    </span>
  );
        ...
        <div className="flex flex-wrap items-center gap-3 p-3 bg-page/50 border border-border rounded-lg">
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              <select
                value={strategy}
                onChange={(e) =>
                  onStrategyChange(e.target.value as StrategyValue)
                }
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              <select
                value={reason}
                onChange={(e) => onReasonChange(e.target.value as ReasonValue)}
                className="appearance-none pl-3 pr-8 py-1.5 border border-border rounded text-[12px] bg-card text-text focus:outline-none focus:ring-1 focus:ring-accent cursor-pointer"
              >
        ...
        <tr key={i} className="border-b border-border/60 last:border-0">
          {Array.from({ length: colSpan }).map((_, j) => (
            <td key={j} className="px-6 py-4">
              <div
                className="h-4 bg-border/60 rounded animate-pulse"
